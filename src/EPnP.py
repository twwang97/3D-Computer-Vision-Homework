...
from src.gaussOptimization import * 
...
class EPnP(object):
    ...
    def optimize_betas_gauss_newton(self, Kernel, Beta0, Xw, U):
        n = len(Beta0)
        Beta_opt, error_opt = gauss_newton(Kernel, self.Cw, Beta0)
        X = np.zeros((12))
        for i in range(n):
    ...
    def dim_kerM(self, dimker, Km, Xworld, Ximg_pix, L6_10):
        ...
        if dimker == 2:
            L = compute_L6_3(L6_10)
            dsp = compute_rho(self.Cw)
